...
function formatTime(num){
    let [number,deciaml="0"] = num.toString().split('.');
    let sign = '+';
    if(number.charAt(0)=='-'){
    ...
    if(Number(deciaml)<10){
        deciaml=deciaml+'0';
    }
    return sign+number+":"+deciaml;
}
let idCurr=1;
for(let i=0;i<countryCapital.length; i++){
    let index = countryUtc.findIndex((val)=>val.name=== countryCapital[i].country);
    
    if(index!=-1){
        let utc = formatTime(countryUtc[index].timezone_offset);
...
fs.writeFile("./countries.js",JSON.stringify(countries),'utf-8',(err)=>{
    if(err) throw err;
    console.log("success");
});
